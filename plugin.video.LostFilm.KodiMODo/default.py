...
sid_file = os.path.join(xbmc.translatePath('special://temp/'), 'plugin.video.LostFilm.KodiMODo.cookies.sid')
...
addon = xbmcaddon.Addon(id='plugin.video.LostFilm.KodiMODo')
__settings__ = xbmcaddon.Addon(id='plugin.video.LostFilm.KodiMODo')
...
def openTorrent(url, userStorageDirectory):
	...
	for contentId, contentFile in enumerate(torrent.getContentList()):
		...
		contentList.append((fileTitle, str(contentId)))
	contentList = sorted(contentList, key=lambda x: x[0])
	if len(contentList)<2:
		playTorrent(url, userStorageDirectory,0)
...
def playTorrent(torrentUrl, StorageDirectory, contentId):
		...
		if 0 != len(torrentUrl):
			...
			playlist.add('file:///' + torrent.getFilePath(contentId).replace("//","/"), listitem)
			#progressBar.close()
			#xbmc.Player().play(playlist)
			print 'file:///' + torrent.getFilePath(contentId).replace("\\\\","\\")
			xbmc.Player().play('file:///' + torrent.getFilePath(contentId).replace("\\\\","\\"))
			progressBar.close()
			#time.sleep(15)
	...
	if out=='Ok':
		...
		for k,v in TSplayer.files.iteritems():
			...
			try:
				...
				pass
			#li.addContextMenuItems([('Hайти на KrasFS', 'XBMC.ActivateWindow(10025,"plugin://plugin.video.LostFilm.KodiMODo/?mode=KFS&text='+k+'")', 'XBMC.ActivateWindow(10025,"plugin://plugin.video.LostFilm.KodiMODo/?mode=OpenCat&text=0&file='+url+'&img='+img+'")')])
...
def AllList(L):
		for i in L:
			...
			purl = sys.argv[0] + '?mode=OpenPage'\
				...
				+ '&title=' + urllib.quote_plus(Title)\
				+ '&text=' + urllib.quote_plus('0')
			#listitem.addContextMenuItems([('Следить', 'Container.Update("plugin://plugin.video.LostFilm.KodiMODo/?mode=Getlist&url='+urllib.quote_plus(row_url)+'&title='+urllib.quote_plus(dict['title'])+'")'),])
			lf_name=mfindal(dict['title'],'(',')')[0][1:]
			lf_url='plugin://plugin.video.LostFilm.KodiMODo/?mode=Getlist&url='+urllib.quote_plus(row_url)+'&title='+urllib.quote_plus(lf_name)
...
if mode == None or mode == "None":
	#LF()
	#playTorrent("http://d.rutor.org/download/429448", "D:\\TS\\")
	GET_N()
	xbmcplugin.setPluginCategory(handle, PLUGIN_NAME)
	xbmcplugin.endOfDirectory(handle)
...
elif mode == 'playTorrent':
		ind  = eval(urllib.unquote_plus(params["ind"]))
		print "ind  "+str(ind)
		DownloadDirectory = __settings__.getSetting("DownloadDirectory")
		if DownloadDirectory=="":DownloadDirectory=LstDir
