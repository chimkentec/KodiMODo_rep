...
_addon_name 	= 'plugin.audio.101.KodiMODo'
...
def NewListItem():
	...
	item = xbmcgui.ListItem(params['title'],'plugin.audio.101.KodiMODo-'+params['id']+'-'+params['playmode'], iconImage = '', thumbnailImage = '')
	item.setInfo(type="Music", infoLabels={"Title":title})
	return item
...
def addfavorite(params):
	...
	Data = Get_url(url_, Headers)	
	xbmcgui.Dialog().ok('plugin.audio.101.KodiMODo', Data)
...
def addcurrenttrack(params):
	if not xbmc.Player().isPlayingAudio():
		xbmc.executebuiltin("XBMC.Notification(plugin.audio.101.KodiMODo, Audio не проигрывается, 5000)")
		return
	...
	try:
		...
		cur=cur.split('-')
		if cur[0] != 'plugin.audio.101.KodiMODo': raise
		cur_id=cur[1]
		cur_pm=cur[2]
	except:
		xbmc.executebuiltin("XBMC.Notification(plugin.audio.101.KodiMODo, Ошибка или Радио не 101.ru, 5000)")
		return
